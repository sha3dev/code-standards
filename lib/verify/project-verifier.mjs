...
import { buildVerificationResult, createVerificationIssue, isFileSelected, normalizeRequestedFiles, shouldRunRule } from "./issue-helpers.mjs";
...
export async function verifyProject(targetPath, options = {}) {
  ...
  for (const ruleId of contract.rules.map((rule) => rule.id)) {
    addCheckedRuleId(checkedRuleIds, options.onlyRuleIds, ruleId);
  }

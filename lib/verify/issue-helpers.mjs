...
export function buildVerificationResult(issues, checkedRuleIds, checkedFiles) {
  const deterministicIssueCount = issues.filter((issue) => issue.verificationMode === "deterministic").length;
  const heuristicIssueCount = issues.filter((issue) => issue.verificationMode === "heuristic").length;
  const auditIssueCount = issues.filter((issue) => issue.verificationMode === "audit").length;
  return {
    ok: issues.length === 0,
    issues,
    summary: {
